...
const signup = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const error: ValidationError | undefined = user_validate.signup.validate({ name, email, password, confirm_pass }, { abortEarly: false }).error;
        if (error) {
        ...
        await db.user.upsert({
            where: { email },
            update: {
                name, password: bcrypt.hashSync(password, 10),
                verfication_code: {
                    update: {
