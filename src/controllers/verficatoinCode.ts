...
const verfication_code = {
    generateCode: async (req: Request, res: Response): Promise<void> => {
        try {
            const schema: ObjectSchema = Joi.object({ email: Joi.string().email().pattern(/@gmail\.com$/).required() });
            const { email } = req.body as Record<string, string>;
            const error: ValidationError | undefined = schema.validate({ email }, { abortEarly: false }).error;
            if (error) {
    ...
    verifyCode: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const error: ValidationError | undefined = schema.validate({ email, code }, { abortEarly: false }).error;
            if (error) {
                throw new Error(error.details[0].message);
            }
